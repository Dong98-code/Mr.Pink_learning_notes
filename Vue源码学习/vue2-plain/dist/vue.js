(function (global, factory) {
  ...
  */
  ...
  function patchProps(el, oldProps, props) {
    ...
    var newStyle = (props === null || props === void 0 ? void 0 : props.style) || {}; // 新的style
    // 样式移除
  ...
  function updateChildren(el, oldChildren, newChildren) {
    ...
    var oldStartIndex = 0,
        ...
        newStartIndex = 0,
        newEndIndex = newChildren.length - 1; // 分别指向四个vnode

    var oldStartVnode = oldChildren[oldStartIndex],
        oldEndVnode = oldChildren[oldEndIndex];
    var newStartVnode = newChildren[newStartIndex],
        newEndVnode = newChildren[newEndIndex]; // 乱序比较时使用的map

    var map = makeIndexByKey(oldChildren); //根据旧的孩子们 的key生成其对应的index;
    ...
    while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
      ...
      if (!oldStartVnode) {
        ...
        if (isSameNode(oldStartVnode, newStartVnode)) {
          // 递归比较子节点
          patchVnode(oldStartVnode, newStartVnode); // 比较结束， 移动指针；
          ...
          insertBefore(el, oldEndVnode.el, oldStartVnode.el); // 旧的尾指针前移， 新的头指针后
          ...
          insertBefore(el, oldStartVnode.el, oldEndVnode.el.nextSibling); // 旧的头指针后移
          ...
          // 有值的话移动节点的索引
          var moveIndex = map[newStartVnode.key]; //赵新节点的开始节点的key

          if (moveIndex !== undefined) {
            var moveVnode = oldChildren[moveIndex]; //
            // 插入到头节点之前

            insertBefore(el, moveVnode, oldStartVnode.el); // 讲对应的移动的节点置为undefined
    ...
    if (newStartIndex <= newEndIndex) {
      for (var i = newStartIndex; i <= newEndIndex; i++) {
        ...
        var anchor = (_newChildren = newChildren[newStartIndex + 1]) === null || _newChildren === void 0 ? void 0 : _newChildren.el; // 如果没有的话，默认在结尾插入新节点
    ...
    if (oldStartIndex <= oldEndIndex) {
      for (var _i = oldStartIndex; _i <= oldEndIndex; _i++) {
        oldChildren[_i] && removeChild(el, oldChildren[_i].el); //从el身上移除默认的el
      }
    }
  }
  ...
  function patchVnode(oldVnode, newVnode) {
    ...
    if (!isSameNode(oldVnode, newVnode)) {
      // newNode为一个vnode
      var newEl = createEle(oldVnode); // 根据虚拟节点，创建新的element节点
    ...
    patchProps(el, oldVnode.props, newVnode.props); // 比较儿子节点；
    //获取儿子

    var oldChildren = oldVnode.children || []; // 默认 vnode的儿子为数组形式

    var newChildren = newVnode.children || [];
    var oldLen = oldChildren.length,
        newLen = newChildren.length; // 分情况1. 两方都有儿子节点 2.一方有儿子，新的有，旧的没有；新的没有，旧的有；3.新的没有，旧的也没有；
    ...
    Object.defineProperties(Vue.prototype, {
      ...
      _update: {
        value: function _update(vnode) {
          ...
          var el = vm.$el; // vm.$el = patch(el, vnode)
